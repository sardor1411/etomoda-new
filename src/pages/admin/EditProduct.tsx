...
import { updateProductComplete, getProductComplete } from '../../services/products';
...
export function EditProduct() {
  ...
  useEffect(() => {
    if (titleWatch && !isDirty && !initialLoading) {
      setValue('slug', generateSlug(titleWatch), { shouldValidate: true });
    }
  }, [titleWatch, setValue, isDirty, initialLoading]);

  useEffect(() => {
    getCategories().then(setCategories).catch(console.error);
    if (id) {
      getProductComplete(id).then(({ product, images, colors, sizes }) => {
        reset({
          title: product.title,
          slug: product.slug,
          ...
          discount_end_date: product.color_images?.discount_end_date || '',
          stock: product.stock,
          is_featured: product.is_featured,
          is_new: product.is_new,
          is_active: product.is_active,
          category_id: product.category_id,
          category: (product.category as any) || 'unisex',
        ...
        setSelectedColors(colors.map((c: any) => {
          const matchedImage = images.find((img: any) => img.image_url === c.image_url);
          return { name: c.color_name, hex: c.color_hex, stock: c.stock, image_id: matchedImage ? matchedImage.id : undefined };
        }));
        setSelectedSizes(sizes.map((s: any) => ({ size: s.size, stock: s.stock })));
      }).catch(err => {
        console.error(err);
        setErrorMsg("Mahsulotni yuklashda xatolik yuz berdi");
      }).finally(() => {
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      ...
      if (id) {
        await updateProductComplete(id, productData, dbImages, colorsData, sizesData);
      } else {
        // Technically this is EditProduct, so id should always exist.
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto p-4 md:p-8 space-y-8 pb-24">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Mahsulotni Tahrirlash</h1>
            <p className="text-gray-500">Mahsulot ma'lumotlarini o'zgartiring</p>
          </div>
        </div>
      </div>
                      ...
                      {selectedColors.map((color) => (
                        <div key={color.name} className="flex items-center gap-4 bg-gray-50/50 p-3 rounded-xl border border-gray-100">
                          ...
                          <div className="flex items-center gap-2 flex-grow">
                            <Label className="text-gray-500 whitespace-nowrap">Zaxira:</Label>
                            <Input 
                              type="text"
